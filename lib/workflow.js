...
function detectChecklistSignals({ body = "", subject = "", attachments = [] } = {}) {
  const text = compactText(subject, body);
  const attachmentNames = attachmentText(attachments);
  const hasAnyAttachment = hasAttachment(attachments);
  const saysAttached = /\b(attached|attachment|enclosed|sent over|sending over|shared|uploaded)\b/.test(text);
  const mentionsInvoice = /\b(invoice|invoiced|billing|payment request|receipt|purchase order|po number)\b/.test(text);
  const invoiceAttachment = /\b(invoice|receipt|billing|payment)\b/.test(attachmentNames);
  const contentAttachment = /\b(content|video|rushes|edit|final|draft|asset|creative|ugc)\b/.test(attachmentNames);

  return {
    accepted: /\b(accepted|accept|happy to go ahead|sounds good|yes|confirm|confirmed|approved to proceed|go ahead)\b/.test(text),
    declined: /\b(no|decline|declined|not interested|pass|can't|cannot|not able to|not going ahead)\b/.test(text),
    invoiceReceived: mentionsInvoice || invoiceAttachment || (hasAnyAttachment && saysAttached && /\binvoice\b/.test(text)),
    contentReceived: /\b(content|video|rushes|files|uploaded|drive|dropbox|wetransfer|we transfer|assets|edit|final cut|draft)\b/.test(text) || contentAttachment,
    approvalReceived: /\b(approved|approve|approval|looks good|all good|fine to publish|happy for you to publish|signed off|sign off)\b/.test(text),
    question: text.includes("?") || /\b(question|clarify|wondering|can you|could you|how do)\b/.test(text)
  };
}

function intentFromSignals(signals) {
  if (signals.declined) return "declined";
  if (signals.approvalReceived) return "approval";
  if (signals.contentReceived) return "content sent";
  if (signals.invoiceReceived) return "invoice sent";
  if (signals.accepted) return "accepted";
  return "question";
}

function addChecklistUpdate(items, key, label, previous, value) {
  if (previous === value) return;
  items.push({ key, label, previous, value });
}

export function reviewInboundEmail(creator, { body = "", subject = "", attachments = [] } = {}) {
  const signals = detectChecklistSignals({ body, subject, attachments });
  const intent = intentFromSignals(signals);
  const update = { ...creator };
  const checklist_updates = [];

  if (
    creator.workflow_stage === "First Touch Outreach" &&
    (signals.accepted || signals.question) &&
    !signals.invoiceReceived &&
    !signals.contentReceived &&
  ...
  if (signals.accepted) {
    addChecklistUpdate(checklist_updates, "offer_status", "Offer accepted", update.offer_status, "accepted");
  ...
  if (signals.declined) {
    addChecklistUpdate(checklist_updates, "offer_status", "Offer declined", update.offer_status, "declined");
  ...
  if (signals.invoiceReceived) {
    addChecklistUpdate(checklist_updates, "invoice_status", "Invoice received", update.invoice_status, "received");
  ...
  if (signals.contentReceived) {
    addChecklistUpdate(checklist_updates, "content_status", "Content received", update.content_status, "received");
    update.content_status = "received";
    update.approval_status = update.approval_status === "approved" ? "approved" : "pending";
    update.workflow_stage = "Thank You + Final Check";
    update.next_action = "Review content and send final check";
  }

  if (signals.approvalReceived) {
    addChecklistUpdate(checklist_updates, "approval_status", "Approval received", update.approval_status, "approved");
    update.approval_status = "approved";
    update.workflow_stage = "Post-Invoice Chase";
    update.next_action = "Request remaining invoice";
  }

  if (signals.question && checklist_updates.length === 0) {
    update.next_action = "Reply to creator question";
  }

  const summary = checklist_updates.length
    ? `Updated ${checklist_updates.map((item) => item.label.toLowerCase()).join(", ")}.`
    : "No checklist status changed.";
