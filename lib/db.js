...
export const SHARED_STAGE_DOCUMENTS_KEY = "__shared_stage_documents__";
...
export function listCreatorsById() {
  return ensureDb()
    .prepare("SELECT * FROM creators ORDER BY id")
    .all()
    .map(normalize);
}
...
export function getSyncMeta(key) {
  return ensureDb().prepare("SELECT value FROM sync_meta WHERE key = ?").get(key)?.value || null;
}
...
function normalizeCalendarId(calendarId = "primary") {
  return String(calendarId || "primary").trim() || "primary";
}

function normalizeCalendarNote(row) {
  if (!row) return null;
  return {
    ...row,
...
export function listCalendarNotes(calendarId = "primary") {
  return ensureDb()
    .prepare("SELECT * FROM calendar_notes WHERE calendar_id = ? ORDER BY updated_at DESC")
    .all(normalizeCalendarId(calendarId))
    .map(normalizeCalendarNote);
}

export function getCalendarNote(calendarId = "primary", eventId) {
  if (!eventId) return null;
  return normalizeCalendarNote(
    ensureDb()
      .prepare("SELECT * FROM calendar_notes WHERE calendar_id = ? AND event_id = ?")
      .get(normalizeCalendarId(calendarId), String(eventId))
  );
}

export function saveCalendarNote(calendarId = "primary", eventId, input = {}) {
  if (!eventId) return null;
  ensureDb()
    .prepare(`
      INSERT INTO calendar_notes (calendar_id, event_id, notes, script, event_data, updated_at)
      VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
    ...
    `)
    .run(
      normalizeCalendarId(calendarId),
      String(eventId),
      String(input.notes || ""),
      String(input.script || ""),
  ...
  return getCalendarNote(calendarId, eventId);
}
