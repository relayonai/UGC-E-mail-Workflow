...
import { applyIntentToCreator, computeNextAction, detectIntent } from "./workflow.js";
...
export function receiveReply(creatorId, { body, from, channel = "manual", provider_message_id = null, received_at = null, subject = "" }) {
  const creator = getCreator(creatorId);
  if (!creator) return null;

  const now = received_at || new Date().toISOString();
  const cleanBody = latestReplyText(body);
  const intent = detectIntent(cleanBody);
  const threadId = matchThread(creator, {});
  const updatedByIntent = applyIntentToCreator(creator, intent);
...
export async function syncInboxReplies() {
  ...
  for (const message of messages) {
    ...
    processed.push({
      ...
      from: message.from,
      subject: message.subject,
      intent: updated.message_history[updated.message_history.length - 1]?.intent
    });
  }
